...
impl Tool for SkillTool {
    ...
    async fn execute(&self, input: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        let params: SkillToolInput = serde_json::from_value(input)
            .map_err(|e| ToolError::Message { message: format!("invalid input: {e}") })?;
        ...
        match params.action.as_str() {
            "list" => {
                ...
                if skills.is_empty() {
                    return Ok(ToolOutput {
                        text: "No skills available. Place `.md` skill files in `.claude/skills/`.".into(),
                        is_error: false,
                        json: None,
                    });
                }
                ...
                for s in &skills {
                    let mark = if active_name.as_deref() == Some(&s.name) { "★" } else { " " };
            ...
            "activate" => {
                ...
                let skill = reg.get(name).cloned().ok_or_else(|| ToolError::Message {
                    message: format!("skill `{name}` not found. Use action=\"list\" to see available skills."),
                })?;
                let prompt_len = skill.prompt.len();
                *self.active.write().await = Some(skill.clone());
                Ok(ToolOutput {
                    text: format!("Skill `/{}` activated ({} chars of expertise loaded).", skill.name, prompt_len),
                    is_error: false,
                    json: None,
                })
            }
            ...
            other => Err(ToolError::Message {
                message: format!("unknown action: {other}. Use `list`, `activate`, or `deactivate`."),
            }),
        }
    }
}
