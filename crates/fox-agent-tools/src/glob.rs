...
impl Tool for GlobTool {
    ...
    fn description(&self) -> &str {
        "Find files by glob pattern. Supports gitignore-aware file walking."
    }
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        let params: GlobInput = serde_json::from_value(input).map_err(|e| ToolError::Message {
            message: format!("invalid glob input: {e}"),
        })?;
        ...
        let results = tokio::task::spawn_blocking(move || glob_blocking(&base, &pattern))
            .await
