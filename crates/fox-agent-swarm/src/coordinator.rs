use fox_agent_tools::{PlanItem, PlanStatus, PlanningStore, VersionedPlan, save_plan_with_store};
...
impl SwarmCoordinator {
        ...
        let next_idx = plan.items.iter().position(|item| {
            item.status == PlanStatus::Pending
                && item
                    .blocked_by
                    .iter()
                    .all(|b| completed_ids.iter().any(|d| d == b))
        })?;
    ...
    pub async fn dm(
        ...
        if !self.workers.read().await.contains_key(to_worker_id) {
            return None;
        }
        let now_secs = SystemTime::now()
            .duration_since(UNIX_EPOCH)
