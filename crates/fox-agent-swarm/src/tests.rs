...
mod swarm_tests {
    ...
    async fn broadcast_and_dm_deliver_messages() {
        ...
        assert!(
            inbox
                .iter()
                .any(|m| m.kind == SwarmMessageKind::Broadcast && m.content == "hello")
        );
        assert!(
            inbox
                .iter()
                .any(|m| m.kind == SwarmMessageKind::Direct && m.content == "private")
        );
    }
    ...
    async fn await_members_waits_until_expected_count() {
        ...
        let workers = coordinator
            .await_members(2, std::time::Duration::from_millis(500))
            .await
            .unwrap();
