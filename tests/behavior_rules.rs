...
fn empty_events_no_violations() {
    ...
    assert!(
        violations.is_empty(),
        "empty events should have no violations"
    );
}
...
fn normal_tool_use_no_errors() {
    ...
    assert!(
        errors.is_empty(),
        "normal tool use should have no errors, got: {errors:?}"
    );
}
...
fn excessive_tool_calls_triggers_violation() {
    ...
    let engine = BehaviorRuleEngine::with_default_rules();
    let violations = engine.check(&events);
    let storm: Vec<_> = violations
        .iter()
        .filter(|v| v.rule_name == "no_repeat_tool_storm")
        .collect();
    assert!(
        !storm.is_empty(),
        "should detect tool storm, got: {violations:?}"
    );
...
fn empty_turn_triggers_warning() {
    ...
    let engine = BehaviorRuleEngine::with_default_rules();
    let violations = engine.check(&events);
    let empty: Vec<_> = violations
        .iter()
        .filter(|v| v.rule_name == "no_empty_turn")
        .collect();
    assert!(
        !empty.is_empty(),
        "should detect empty turn, got: {violations:?}"
    );
}
...
fn orphaned_tool_call_triggers_error() {
    ...
    let engine = BehaviorRuleEngine::with_default_rules();
    let violations = engine.check(&events);
    let orphan: Vec<_> = violations
        .iter()
        .filter(|v| v.rule_name == "tool_output_not_orphaned")
        .collect();
    assert!(
        !orphan.is_empty(),
        "should detect orphaned tool call, got: {violations:?}"
    );
}
