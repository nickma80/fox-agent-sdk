//! 深度场景测试：模拟真实 Agent 任务的完整评估管线。
//!
//! 每个测试用例用 [`MockProvider`] 脚本 + 预置文件状态模拟一次完整的
//! Agent 执行，然后验证评估管线：TaskAssertions → BehaviorRuleEngine → TokenReport。
//!
//! Run:
//! ```bash
...
use fox_agent_core::{
    AgentEvent, DefaultSafetyPolicy, FoxAgentSdkConfig, SafetyConfig, StreamEvent, TaskAssertions,
    TokenReport, Tool, ToolContext, ToolError, ToolOutput, run_task_assertions,
};
use fox_agent_sdk::eval::behavior_rules::{BehaviorRuleEngine, RuleSeverity};
use fox_agent_sdk::{Agent, GovernanceGuard, Harness, MockProvider};
...
impl Tool for FsCreateTool {
    ...
    async fn execute(&self, input: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        std::fs::write(&full, content).map_err(|e| ToolError::Message {
            message: e.to_string(),
        })?;
        Ok(ToolOutput {
            text: format!("Wrote {}", fp),
            is_error: false,
            json: None,
        })
    }
...
impl Tool for FsReadTool {
    ...
    async fn execute(&self, input: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        let fp = input["file_path"].as_str().unwrap_or_default();
        let full = self.root.join(fp.trim_start_matches('/'));
        let content = std::fs::read_to_string(&full).map_err(|e| ToolError::Message {
            message: e.to_string(),
        })?;
        Ok(ToolOutput {
            text: content,
...
impl CmdTool {
    fn new(outputs: HashMap<String, (String, bool)>) -> Self {
        Self { outputs }
    }
}
...
fn tool_use_script(call_id: &str, name: &str, input: Value) -> Vec<StreamEvent> {
    vec![
        ...
        StreamEvent::Usage {
            usage: fox_agent_core::TokenUsage {
                input_tokens: 50,
                output_tokens: 20,
                total_tokens: 70,
    ...
    vec![
        ...
        StreamEvent::Usage {
            usage: fox_agent_core::TokenUsage {
                ...
                total_tokens: (30 + text.len()) as u32,
                cache_read_input_tokens: None,
                cache_creation_input_tokens: None,
            },
        },
...
async fn case_001_create_rust_project_and_compile() {
    ...
    provider.push_script(tool_use_script(
        "c1",
        "bash",
        json!({"command":"cargo init test-project"}),
    ));
    provider.push_script(tool_use_script(
        "c2",
        "bash",
        json!({"command":"cd test-project && cargo build"}),
    ));
    provider.push_script(text_script("Project created and built"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Create a Rust project", &tx)
        .await
        .unwrap();
    drop(tx);
    ...
    let assertions = TaskAssertions {
        file_exists: vec![
            PathBuf::from("test-project/Cargo.toml"),
    ...
    println!(
        "Token: {} input / {} output / {} calls",
        token.total_input, token.total_output, token.api_calls
    );
}
...
async fn case_002_multi_file_edit() {
    ...
    provider.push_script(tool_use_script(
        "c2",
        "write",
        json!({"file_path":"src/main.rs","content":"fn main() { println!(\"new\"); }\n"}),
    ));
    provider.push_script(tool_use_script(
        "c3",
        "read",
    ...
    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Update greeting", &tx)
        .await
        .unwrap();
    drop(tx);
...
async fn case_003_codebase_search() {
    ...
    provider.push_script(tool_use_script(
        "c2",
        "read",
        json!({"file_path":"config.rs"}),
    ));
    provider.push_script(text_script("Found DB_URL"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Search DB_URL", &tx)
        .await
        .unwrap();
    drop(tx);
    let events = drain(&mut rx).await;
    ...
    let has_grep = events
        .iter()
        .any(|ev| matches!(ev, AgentEvent::ToolCallStart { name, .. } if name == "grep"));
    let has_read = events
        .iter()
        .any(|ev| matches!(ev, AgentEvent::ToolCallStart { name, .. } if name == "read"));
...
async fn case_004_git_log_analysis() {
    ...
    let cmd = Arc::new(CmdTool::new(cmd_outputs));
    let grep = Arc::new(GrepTool {
        results: vec!["abc123 Alice".into(), "ghi789 Alice".into()],
    });
    ...
    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Show Alice's commits", &tx)
        .await
        .unwrap();
    drop(tx);
    let events = drain(&mut rx).await;
    ...
    let has_log = events
        .iter()
        .any(|ev| matches!(ev, AgentEvent::ToolCallStart { name, .. } if name == "bash"));
    let has_grep = events
        .iter()
        .any(|ev| matches!(ev, AgentEvent::ToolCallStart { name, .. } if name == "grep"));
...
async fn case_005_error_diagnosis_and_fix() {
    ...
    let mut cmd_outputs = HashMap::new();
    cmd_outputs.insert(
        "cargo build-err".into(),
        ("error[E0282]: type annotations needed".into(), true),
    );
    cmd_outputs.insert(
        "cargo build-ok".into(),
        ("Compiling my-app\nFinished".into(), false),
    ...
    provider.push_script(tool_use_script(
        "c3",
        "write",
        json!({"file_path":"src/main.rs","content":"fn main() { let x: i32 = 1; }\n"}),
    ));
    provider.push_script(tool_use_script(
        "c4",
        "bash",
        json!({"command":"cargo build-ok"}),
    ));
    provider.push_script(text_script("Fixed type annotation"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Fix the build", &tx)
        .await
        .unwrap();
    drop(tx);
    let events = drain(&mut rx).await;
    ...
    assert_eq!(
        tool_names,
        vec!["bash", "read", "write", "bash"],
        "Error→fix→rebuild flow"
    );
...
async fn case_006_permission_denied_recovery() {
    ...
    provider.push_script(tool_use_script(
        "c2",
        "write",
        json!({"file_path":"cleaned.txt","content":"safe cleanup\n"}),
    ));
    provider.push_script(text_script("Used safe alternative"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Clean up safely", &tx)
        .await
        .unwrap();
    drop(tx);
    let events = drain(&mut rx).await;
...
async fn case_007_tool_timeout_handling() {
    ...
    cmd_outputs.insert(
        "sleep".into(),
        ("timeout: command exceeded limit".into(), true),
    );
    ...
    provider.push_script(tool_use_script(
        "c1",
        "bash",
        json!({"command":"sleep 120","timeout":30000}),
    ));
    provider.push_script(tool_use_script(
        "c2",
        "bash",
...
async fn case_008_compaction_behavior() {
    ...
    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Create 5 files", &tx)
        .await
        .unwrap();
    drop(tx);
    let events = drain(&mut rx).await;
    ...
    let violations = engine.check(&events);
    assert!(
        violations.iter().all(|v| v.severity != RuleSeverity::Error),
...
async fn case_009_mcp_tool_routing() {
    ...
    let has_mcp = events.iter().any(
        |ev| matches!(ev, AgentEvent::ToolCallStart { name, .. } if name.starts_with("mcp__")),
    );
...
async fn case_010_subagent_delegation() {
    ...
    impl Tool for SubAgentTool {
        ...
        async fn execute(&self, _input: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
            Ok(ToolOutput {
                text: "artifact:sub-001".into(),
                is_error: false,
                json: Some(json!({"artifact_id":"sub-001"})),
            })
        }
    }
    ...
    let (agent, provider) =
        build_agent(vec![Arc::new(SubAgentTool), Arc::new(ArtifactReadTool)]).await;

    provider.push_script(tool_use_script("c1", "subagent", json!({"task":"search"})));
    provider.push_script(tool_use_script(
        "c2",
        "artifact_read",
        json!({"artifact_id":"sub-001"}),
    ));
    provider.push_script(text_script("Sub-agent results read"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Search codebase", &tx)
        .await
        .unwrap();
    drop(tx);
    let events = drain(&mut rx).await;
    ...
    let has_sub = events
        .iter()
        .any(|ev| matches!(ev, AgentEvent::ToolCallStart { name, .. } if name == "subagent"));
    let has_read = events
        .iter()
        .any(|ev| matches!(ev, AgentEvent::ToolCallStart { name, .. } if name == "artifact_read"));
...
async fn case_011_multi_turn_conversation() {
    ...
    provider.push_script(tool_use_script(
        "c1",
        "write",
        json!({"file_path":"notes.md","content":"Step 1\n"}),
    ));
    provider.push_script(tool_use_script(
        "c2",
        "write",
...
async fn case_012_large_file_read_truncation() {
    ...
    provider.push_script(tool_use_script(
        "c1",
        "read",
        json!({"file_path":"large.log"}),
    ));
    provider.push_script(text_script("Read complete"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Read large.log", &tx)
        .await
        .unwrap();
    drop(tx);
    let events = drain(&mut rx).await;
    ...
    let sizes: Vec<usize> = events
        .iter()
        .filter_map(|ev| match ev {
            AgentEvent::ToolCallEnd { output, .. } => Some(output.text.len()),
            _ => None,
        })
        .collect();
...
async fn case_013_concurrent_tool_execution() {
    ...
    let turn = vec![
        StreamEvent::ToolUse {
            id: "c1".into(),
            name: "write".into(),
            input: json!({"file_path":"a.txt","content":"A"}),
        },
        StreamEvent::ToolUse {
            id: "c2".into(),
            name: "write".into(),
            input: json!({"file_path":"b.txt","content":"B"}),
        },
        StreamEvent::Usage {
            usage: fox_agent_core::TokenUsage {
                input_tokens: 100,
    ...
    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Create two files", &tx)
        .await
        .unwrap();
    drop(tx);
    let _events = drain(&mut rx).await;
...
async fn case_014_plan_todo_consistency() {
    ...
    assert!(violations.iter().all(|v| v.severity != RuleSeverity::Error));
    println!(
        "Plan-todo: plan used, {} plans / {} todos recorded",
        plan.plans().len(),
        todo.todos().len()
    );
}
...
async fn case_015_environment_initialization() {
    ...
    let write = Arc::new(FsCreateTool { root: root.clone() });
    let ls = Arc::new(LsTool {
        entries: vec!["README.md".into(), "src/".into()],
    });
    let read = Arc::new(FsReadTool { root: root.clone() });
    ...
    provider.push_script(tool_use_script(
        "c3",
        "write",
        json!({"file_path":".env","content":"DEBUG=true\n"}),
    ));
    provider.push_script(text_script("Environment ready"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let _ = agent
        .run_once_streaming("Init environment", &tx)
        .await
        .unwrap();
    drop(tx);
    let _events = drain(&mut rx).await;
...
#[tokio::test]
async fn case_016_max_turns_tool_loop_limit() {
    // max_turns=3: agent should stop after 3 tool-loop iterations
    let dir = tempfile::tempdir().unwrap();
    let root = Arc::new(dir.path().to_path_buf());
    let write = Arc::new(FsCreateTool { root: root.clone() });

    let provider = MockProvider::new("max_turns_test");
    let harness = Harness::new(
        FoxAgentSdkConfig {
            safety: SafetyConfig {
                default_policy: DefaultSafetyPolicy::Allow,
    ...
    harness.register_tool(write).await;
    ...
    agent.set_governance(GovernanceGuard::new(fox_agent_core::BudgetConfig {
        max_turns: 3,
        ..Default::default()
    }));

    // Push 5 tool-use scripts; agent should error on iteration 4
    for i in 0..5 {
        provider.push_script(tool_use_script(
            &format!("c{i}"),
            "write",
            json!({"file_path":format!("file_{i}.txt"),"content":"x"}),
        ));
    }
    // Push final text response (won't be reached)
    provider.push_script(text_script("Done"));

    let (tx, mut rx) = tokio::sync::mpsc::channel(128);
    let result = agent.run_once_streaming("Create many files", &tx).await;
    drop(tx);
    let events = drain(&mut rx).await;
